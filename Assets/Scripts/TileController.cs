...
public class TileController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        LustMirror = puzzleScript.getLustMirror();
        gameBoard = puzzleScript.getGameBoardTransform();
    ...
    void OnMouseOver()
    {
       ...
       if (Input.GetMouseButtonDown(0))
       {
		    ...
		    for (int listIndex = 0; listIndex < listOfPieces.Count; listIndex++)
		    {
                ...
                if (isComplete == true)
                {
			        //if the game is complete, deactivate the game
                    gameBoard.gameObject.SetActive(false);
			        //enable the full tile image so the player can't interact w it anymore
